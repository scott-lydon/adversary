...
## 0. Live-run reality (added 2026-05-13, updated 2026-05-14)

> **Honesty note.** Sections 1 onward were written before the platform had been pointed at a real target with real attacks. The 2026-05-14 `--provider live` scan finally produced the first real per-agent dollar numbers. Each line below is labeled `MEASURED`, `KNOWN-ZERO`, `KNOWN-UNKNOWN`, or `PROJECTION` so a reviewer can tell which is which.

### What we actually measured

**Deterministic seed-replay run, 2026-05-13** (`scripts/run-live-evals.py`, `--provider scripted`):

| Quantity | Value | Label |
|---|---|---|
| Live attacks executed against `http://5.161.253.237:8801` | 13 | MEASURED |
| Confirmed exploits (hand-authored seeds, classifier-scored) | 5 (PI-2026-001/003/004, SC-2026-001/004) | MEASURED |
| Defended | 8 | MEASURED |
| Total wall-clock latency across all 13 attacks | 36.9 seconds (2.8 s mean, 1.9 s median, 7.9 s max) | MEASURED |
| Total Co-Pilot `/chat` calls | 19 (PI-2026-004 is multi-turn, contributes 5 calls) | MEASURED |
| Adversary-side LLM dollar cost | **$0.00** | KNOWN-ZERO (`--provider scripted`) |

**Live multi-agent scan, 2026-05-14** (`adversary scan --provider live --max-campaigns 3 --budget-usd 5`):

| Quantity | Value | Label |
|---|---|---|
| Campaigns executed | 3 | MEASURED |
| Attacks generated by Red Team (Llama 3.1 70B via Together AI) | 15 (5 per campaign) | MEASURED |
| Verdicts produced by Judge (Claude Sonnet 4 via Anthropic) | 15 | MEASURED |
| Confirmed exploits (Judge verdict = success) | **0** | MEASURED |
| Defended (Judge verdict = fail) | 15 | MEASURED |
| Red Team total cost | $0.0016 ($0.00011/attack) | MEASURED |
| Red Team tokens (in / out) | 5,077 in / 4,537 out | MEASURED |
| Judge total cost | $0.0383 ($0.0026/verdict) | MEASURED |
| Orchestrator total cost (includes campaign roll-up per BUG_PREVENTION C2) | $0.0830 ($0.0277/campaign) | MEASURED |
| **Total dollar cost, all agents** | **$0.1229** for 3 campaigns | MEASURED |
| **Per-campaign cost (real)** | **$0.041 / campaign** | MEASURED |
| **Per-attack cost (real)** | **$0.0082 / attack** | MEASURED |
| Documentation Agent runs | 0 | KNOWN-ZERO (no SUCCESS verdicts to document this scan) |
| Co-Pilot side LLM dollar cost | not surfaced back to the adversary platform | KNOWN-UNKNOWN (instrumented on the Co-Pilot's own sidecar) |

### Implications for the projections in Sections 1 to 5

The original MVP-tier estimate was **$1.62 per campaign**. Measured cost is **$0.041 per campaign**. That is ~40x cheaper than projected. Two reasons. First, gpt-5-mini orchestrator planning is cheaper than the analysis assumed. Second, Llama 3.1 70B Red Team attack generation is dramatically cheaper per attack than projected (~$0.0001 versus the projected $0.0095). Judge costs are the only line that came in close to projection (~$0.0026 versus projected $0.0030 per verdict).

The corrective action: re-derive Sections 4 and 5's blended cost at scale with these new per-call numbers. The architectural inflection points likely move out by 10x. A 1,000-campaign month now costs roughly $41 of platform spend, not $1,050. A 10,000-campaign month is now in the $400 range, still well below the $4,200 the projection assumed.

The corollary is also useful: cost is no longer the dominant scaling constraint. Throughput against the live target is, because each `/chat` call costs the Co-Pilot's own LLM budget (KNOWN-UNKNOWN to the adversary platform). The real ceiling on campaign volume is the target's tolerance for being attacked, not the adversary's bill.

### Confirmed-exploit reality — autonomous vs hand-authored

The live `--provider live` scan produced **zero** SUCCESS verdicts across 15 Red-Team-generated attacks. The deterministic seed-replay run produced **five** confirmed exploits on the same target on the same day. The honest read: hand-authored, threat-model-informed seed cases probe specific known weak surfaces and find real bugs; Llama 3.1 70B generic attack generation does not yet break through the same target without that context.

Two implications. First, the platform's most useful surface today is the eval / regression path, which encodes human-informed attack expertise. Second, improving the Red Team agent (better seed_examples in the CampaignBrief, stronger novelty hint from the learned-attacks store, target-specific prompt templates) is the highest-leverage next investment.

### What changed because of the live runs

Two adapter / classifier bugs landed in the same push as the 2026-05-13 results (BUG_PREVENTION.md E1 and E2). Zero new bugs surfaced in the 2026-05-14 scan, which is a useful signal that the prior fixes held under the actual orchestrator path.
